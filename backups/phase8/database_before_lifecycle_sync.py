...
def add_incident(incident):
    ...
    # If the same file hash already exists, update the
    # existing incident with the latest scan and response
    # information instead of creating a duplicate incident.
    if new_hash:
        ...
        for index, existing in enumerate(incidents):
            ...
            if existing_hash == new_hash:
                ...
                # Preserve the original incident ID and timestamp.
                incident["incident_id"] = existing.get(
                    "incident_id",
                    incident.get("incident_id")
                )

                incident["timestamp"] = existing.get(
                    "timestamp",
                    incident.get("timestamp")
                )

                # Preserve previous status history if present.
                if existing.get("status_history"):
                    incident["status_history"] = existing[
                        "status_history"
                    ]

                # Replace the old incident with the latest
                # scan/response information.
...
def update_incident_status(
    ...
    for incident in incidents:
        ...
        if incident.get("incident_id") == incident_id:
            ...
            expected_status = lifecycle.get(old_status)
