...
async function cleanSystem() {
    ...
    if (!confirm("Are you sure you want to clean the incident history?")) {
        return;
    }
    ...
    try {
        ...
        message.innerHTML = "🧹 Cleaning incident history...";
        ...
        if (!response.ok) {
            throw new Error("Failed to clean incident history");
        }

        const result = await response.json();

        // Clear dashboard data immediately
        allIncidents = [];

        if (container) {
            container.innerHTML =
                "<p>✅ Incident history cleaned. No incidents found.</p>";
        }
        ...
        message.innerHTML =
            "✅ Incident history cleaned successfully.";
